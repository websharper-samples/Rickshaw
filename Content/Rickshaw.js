...
{
 "use strict";
 var Global,Rickshaw,Client,SC$1,WebSharper,Operators,Object,Obj,UI,HtmlModule,attr,JavaScript,JS,Arrays,EventTarget,Doc,AttrProxy,Client$1,Templates,Pervasives,DomUtility,Attrs,Node,Collections,Dictionary,Unchecked,View,Elt,Array,SC$2,HashSet,Seq,WindowOrWorkerGlobalScope,SC$3,Docs,DocElemNode,CharacterData,Updates,List,T,DictionaryUtil,Prepare,Slice,Enumerator,KeyCollection,An,Settings,Abbrev,Mailbox,Attrs$1,Dyn,Fresh,Snap,SC$4,Strings,Docs$1,RunState,NodeSet,Anims,SC$5,T$1,SC$6,Concurrency,AppendList,HashSetUtil,SC$7,Easing,AsyncBody,SC$8,CT,HashSet$1,Scheduler,CancellationTokenSource,DomNodes,Queue,Error,OperationCanceledException,Lazy,SC$9,LazyExtensionsProxy,LazyRecord,Math,Fixtures,Color,Graph,RangeSlider,Behavior,Series,Axis,Time,Number,IntelliFactory,Runtime,console,$,Date;
 Global=self;
 Rickshaw=Global.Rickshaw=Global.Rickshaw||{};
 Client=Rickshaw.Client=Rickshaw.Client||{};
 SC$1=Global.StartupCode$Rickshaw$Client=Global.StartupCode$Rickshaw$Client||{};
 ...
 Math=Global.Math;
 Fixtures=Rickshaw&&Rickshaw.Fixtures;
 Color=Rickshaw&&Rickshaw.Color;
 Graph=Rickshaw&&Rickshaw.Graph;
 RangeSlider=Graph&&Graph.RangeSlider;
 Behavior=Graph&&Graph.Behavior;
 Series=Behavior&&Behavior.Series;
 Axis=Graph&&Graph.Axis;
 Time=Fixtures&&Fixtures.Time;
 Number=Fixtures&&Fixtures.Number;
 ...
 SC$1.$cctor=function()
 {
  var $1,$2,$3,palette,diagram,series,r,r$1,r$2,r$3,r$4,r$5,r$6,graph,r$7,preview,range,r$8,timeline,annotator,leg,graphlegend,smooth,r$9,r$10,messages,r$11,form,chart,content,seriesdata,i,$4,randata,i$1,$5;
  SC$1.$cctor=Global.ignore;
  function addannotation(a$1)
  {
   if(messages.length>0&&(a$1||Math.random()>=0.95))
    {
     annotator.add(Arrays.get(Arrays.get(seriesdata,2),Arrays.get(seriesdata,2).length-1).x,messages.shift());
     annotator.update();
    }
  }
  function a()
  ...
  seriesdata=[];
  for(i=1,$4=9;i<=$4;i++)seriesdata.push([]);
  randata=new Fixtures.RandomData(150);
  for(i$1=1,$5=70;i$1<=$5;i$1++)randata.addData(seriesdata);
  $3=(palette=new Color.Palette({
   scheme:"classic9"
  }),(diagram=Doc.Element("div",[AttrProxy.Create("id","diagram")],[]),(series=[(r={
   data:Arrays.get(seriesdata,0)
  },r.color=palette.color(),r.name="Moscow",r),(r$1={
   data:Arrays.get(seriesdata,1)
  },r$1.color=palette.color(),r$1.name="Shanghai",r$1),(r$2={
   data:Arrays.get(seriesdata,2)
  },r$2.color=palette.color(),r$2.name="Amsterdam",r$2),(r$3={
   data:Arrays.get(seriesdata,3)
  },r$3.color=palette.color(),r$3.name="Paris",r$3),(r$4={
   data:Arrays.get(seriesdata,4)
  },r$4.color=palette.color(),r$4.name="Tokyo",r$4),(r$5={
   data:Arrays.get(seriesdata,5)
  },r$5.color=palette.color(),r$5.name="London",r$5),(r$6={
   data:Arrays.get(seriesdata,6)
  },r$6.color=palette.color(),r$6.name="New York",r$6)],(graph=new Rickshaw.Graph((r$7={
   element:diagram.elt,
   series:series
  },r$7.width=900,r$7.height=500,r$7.renderer="area",r$7.stroke=true,r$7.preserve=true,r$7)),(graph.render(),preview=Doc.Element("div",[AttrProxy.Create("id","preview")],[]),range=new RangeSlider.Preview({
   element:preview.elt,
   graph:graph
  }),new Graph.HoverDetail((r$8={
   graph:graph
  },r$8.xFormatter=function(x)
  {
   return(new Global.Date(x*1000)).toDateString();
  },r$8)),timeline=Doc.Element("div",[AttrProxy.Create("id","timeline")],[]),annotator=new Graph.Annotate({
   graph:graph,
   element:timeline.elt
  }),leg=Doc.Element("div",[AttrProxy.Create("id","legend")],[]),graphlegend=new Graph.Legend({
   graph:graph,
   element:leg.elt
  }),new Series.Toggle({
   graph:graph,
   ...
   graph:graph,
   legend:graphlegend
  }),new Series.Highlight({
   graph:graph,
   legend:graphlegend
  }),smooth=Doc.Element("div",[AttrProxy.Create("id","smoother")],[]),new Graph.Smoother({
   graph:graph,
   element:smooth.elt
  }),(new Axis.Time((r$9={
   graph:graph
  },r$9.ticksTreatment="glow",r$9.timeFixture=new Time.Local(),r$9))).render(),(new Axis.Y((r$10={
   graph:graph
  },r$10.ticksTreatment="glow",r$10.ticksFormat=function(a$1)
  {
   return Number.formatKMBT(a$1);
  },r$10))).render(),messages=["Changed home page welcome message","Minified JS and CSS","Changed button color from blue to green","Refactored SQL query to use indexed columns","Added additional logging for debugging","Fixed typo","Rewrite conditional logic for clarity","Added documentation for new methods"],Global.setInterval(function()
  {
   randata.removeData(seriesdata);
   randata.addData(seriesdata);
   graph.update();
  },3000),addannotation(true),(new Axis.Time((r$11={
   graph:Arrays.get(range.previews,0)
  },r$11.ticksTreatment="glow",r$11.timeFixture=new Time.Local(),r$11))).render(),form=Doc.Element("div",[AttrProxy.Create("id","side-panel"),AttrProxy.Create("style","float: left")],[leg,smooth]),chart=Doc.Element("div",[AttrProxy.Create("style","float: left; margin: 10px")],[diagram,timeline,preview]),content=Doc.Element("div",[AttrProxy.Create("id","content"),AttrProxy.Create("style","float: left")],[]),content.elt.appendChild(chart.elt),content.elt.appendChild(form.elt),Templates.LoadLocalTemplates(""),Doc.RunById("main",content))))));
  SC$1.Main=$3;
 };
